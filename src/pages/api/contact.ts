...
const generateEmailContent = (data: { [s: string]: unknown; } | ArrayLike<unknown>) => {
  const stringData = Object.entries(data).reduce(
    (str, [key, val]) => str += `${CONTACT_MESSAGE_FIELDS[key as keyof typeof CONTACT_MESSAGE_FIELDS]}: \n${val} \n \n`, "");
...
const handler = async (req: NextApiRequest, res: NextApiResponse<ResponseData>) => {
  if (req.method === 'POST') {
    ...
    if (!data.username || !data.email || !data.phone || !data.message) {
      return res.status(400).json({ message: 'Bad request' } as ResponseData)
    }
    ...
    try {
      ...
      return res.status(200).json({ success: true } as ResponseData)
    } catch (error: any) {
      return res.status(400).json({ message: error.message } as ResponseData)
    }
  }
}
